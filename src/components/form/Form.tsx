...
const Form: React.FunctionComponent<FormProps> = ({
  ...
  const onSelectLocation = async (obj: { label: string; value: any }) => {
    ...
    const { lat: latitude, lng: longitude } =
      geocodedAddress[0].geometry.location.toJSON();
    setAddress(obj);
      ...
      <div className="form-container">
        ...
        >
          <GooglePlacesAutocomplete
            selectProps={{
              value: address,
              onChange: (obj: { label: string; value: any }) =>
          ...
          <SelectMenu
            ...
            onSelect={(item: SelectMenuItem) =>
              setSelectedEmergencyType(
                emergencyTypes.find((type) => type.title === item.label) ?? null
              )
            }
            closeOnSelect
          >
            <Button>{selectedEmergencyType?.title || "Selecteer"}</Button>
          </SelectMenu>
        </FormField>
        <br />
        <FormField
        ...
        >
          <SelectMenu
            ...
            onSelect={(item) => {
              // TODO: via id
              const mappedQualification = qualifications.find(
                (q) => q.name === item.label
              );
            ...
            onDeselect={(item) => {
              const selectedItems = selectedQualifications.filter(
                (q) => q.name !== item.label
              );
        ...
        >
          <Textarea
            onChange={(input: ChangeEvent<HTMLTextAreaElement>) =>
              setAction(input.target.value)
            }
          />
        </FormField>
        <br />
